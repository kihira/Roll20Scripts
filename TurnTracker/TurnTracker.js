let TurnTracker = (() => {
    ...
    function handleInput(msg) {
        if (msg.type !== "api" || !msg.content.startsWith("!turntracker"))
            return;
        ...
        switch (tokenized[1]) {
            ...
            case "claim":
                if (args.length === 0)
                    sendChat("", "/w player|" + msg.playerid + " Missing token id");
                else
                    claimSlot(msg.playerid, args[0]);
                break;
        }
    }
    function getMarker() {
    ...
    function getPlayerTokens() {
        ...
        return filterObjs((value) => {
            if (value.get("type") !== "graphic" || value.get("subtype") !== "token")
                return false;
            if (!value.get("represents") || !value.get("controlledby"))
                return false;
    ...
    function turnOrderChange() {
        ...
        if (turnOrder.length === 0)
            return;
        const current = _.first(turnOrder);
        switch (current.custom) {
            case "PC":
                _.each(getPlayerTokens(), (value) => {
                    sendChat("", `/w player|${value.get("controlledby")} [Claim Slot](!turntracker claim ${value.get("id")})`);
                });
                break;
            case "NPC":
                let message = "";
...
//# sourceMappingURL=TurnTracker.js.map