let TurnTracker = (function () {
    ...
    function claimSlot(playerId, tokenId) {
        ...
        let pRatio = parseInt(previousToken.get('width')) / parseInt(previousToken.get('height'));
        let cRatio = parseInt(currentToken.get('width')) / parseInt(currentToken.get('height'));
        ...
        if (state.TurnMarker.announcePlayerInTurnAnnounce) {
            let characterId = currentToken.get('represents');
            if (characterId) {
                let Char = getObj('character', characterId);
    ...
    function turnOrderChange() {
        ...
        let current: TurnOrderEntry = _.first(turnOrder);
